...
void AGALAGA_USFXPawn::FireShot(FVector FireDirection)
{
    ...
    // If it's ok to fire again and we have bullets left
   // if (bCanFire && NumBullets > 0)
   // {
   //     // If we are pressing fire stick in a direction
   //     if (FireDirection.SizeSquared() > 0.0f)
   //     {
   ...
   //         bCanFire = false;
   //         NumBullets--; // Decrease the number of bullets
			//
   //        
   //         FString Message = FString::Printf(TEXT("BALAS RESTANTES: %d"), NumBullets);
   //         GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Yellow, Message);

			////FString Message2 = FString::Printf(TEXT("VIDAS RESTANTES: %d"), RemainingLives);
